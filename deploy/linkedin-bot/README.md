...
- Workflow `.github/workflows/deploy-linkedin-bot.yml`: builda imagem no GHCR e faz deploy via SSH no host `191.252.210.83`.
...
## Manual enxuto para operação em VPS com interface gráfica

Se você quer apenas o passo a passo direto (sem contexto extra), veja: `deploy/linkedin-bot/MANUAL_LOGIN_LINKEDIN.md`.

## Como rodar com sessão já logada (novo fluxo recomendado)

Com interface gráfica no VPS, basta manter o profile do Chrome autenticado no LinkedIn.
...
2. Garanta que o diretório de profile exista:

```bash
mkdir -p linkedin_chrome_profile && chmod 777 linkedin_chrome_profile
```

3. Execute **um único comando** no terminal da GUI do VPS:
...
Isso significa que a sessão salva no profile expirou ou não foi montada corretamente.

Checklist rápido:

1. Confirme volume e variáveis:

```bash
docker compose exec linkedin-bot env | grep -E 'LINKEDIN_CHROME_USER_DATA_DIR|LINKEDIN_CHROME_PROFILE'
```

2. Se quiser fallback automático por credenciais, defina:
